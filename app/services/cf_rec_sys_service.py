...
def generate_recommendations(
    ...
    for user_id in user_item_matrix.index:
        ...
        similar_users = (
            user_similarities_df.loc[user_id].nlargest(10).index[1:]
        )  # 자기 자신 제외
    ...
    for user_id in user_item_matrix.index:
        ...
        recommendations[user_id] = [
            (book, recommended_books[book]) for book in recommended_books.index
        ]
...
def cf_recommendation(db: Session) -> None:
    ...
    try:
        likes_query = (
            ...
            .filter(Child.is_active == True)  # 활성 사용자만 포함
            .all()
        )
        likes_data = [
