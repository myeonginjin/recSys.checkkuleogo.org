...
def calculate_cosine_similarity(user_item_matrix: pd.DataFrame) -> pd.DataFrame:
    """코사인 유사도를 계산하는 함수"""
    return pd.DataFrame(cosine_similarity(user_item_matrix), index=user_item_matrix.index, columns=user_item_matrix.index)


def generate_recommendations(user_item_matrix: pd.DataFrame, user_similarities_df: pd.DataFrame) -> dict:
    """추천 결과를 생성하는 함수"""
    recommendations = {}
...
def cf_recommendation(db: Session) -> None:
    ...
    try:
        ...
        likes_data = [(BookLike.book_idx, BookLike.child_idx, BookLike.is_like) for BookLike in likes_query]
