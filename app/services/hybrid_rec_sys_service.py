...
from models.schemas import Book, Recommend
    ...
    try:
        ...
        for user in all_users:
            ...
            for book, cf_score in recommendations_1.get(user, []):
                book_scores[book] += cf_score * 0.5  # 가중치 0.5

            # 두 번째 추천 시스템 점수 반영
            for book, cs_score in recommendations_2.get(user, []):
                book_scores[book] += cs_score * 0.5  # 가중치 0.5


            # 공통 책에 추가 가중치 부여
            common_books = set(book for book, _ in recommendations_1.get(user, [])) & \
                           set(book for book, _ in recommendations_2.get(user, []))
            for book in common_books:
                cf_score = next(score for b, score in recommendations_1[user] if b == book)
                cs_score = next(score for b, score in recommendations_2[user] if b == book)
                
                # 공통 책의 가중 평균 점수 계산
                final_score = (0.5 * cf_score) + (0.5 * cs_score)
                book_scores[book] += final_score
